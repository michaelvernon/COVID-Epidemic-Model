...
disp(['SEIQRDP 7-stavov� homog�nn� infektologick� model bez vit�lnej dynamiky'])
disp(['(Predikcia s parametrami na z�klade dostupn�ch �dajov.)'])
disp(['----------------------------'])
disp(['Overen� pr�pady:             ',num2str(InfectedTomorrow),' (do konca dna)'])
disp(['Nov� overen� pr�pady:         ',num2str(InfectedTomorrow-(I(end)+D(end)+R(end))),' (do konca dna)'])
disp(['Celk. pocet infekcn�ch:     ',num2str(InfectedTomorrow+InfectingTomorrow),' (celkov� akt�vne infekcie, odhad, ',num2str(InfectingTomorrow),' mimo testov)'])
disp(['Pocet nakazen�ch:            ',num2str(ExposedTomorrow),' (v inkubacnej dobe, odhad)'])
[valMax indMax]=max(X(:,4));
disp(['Maximum akt�vnych infekci�: ',num2str(round(valMax)),' (pre celu vlnu ochoreni, overen�ch)'])
disp(['Vrchol overen�ch infekci�:    ',datestr(d1+fitBegin+indMax)])
disp(['Infikovan�:                 ',num2str(round(max(X(:,5)))+round(max(X(:,6)))),' (pre celu vlnu ochoreni)'])
disp(['Vyliecen�:                  ',num2str(round(max(X(:,5)))),' (pre celu vlnu ochoreni)'])
disp(['�mrtia:                       ',num2str(round(max(X(:,6)))),' (pre celu vlnu ochoreni)'])
disp(['Koniec infekci�:            ',datestr(d1+fitBegin+min(find(X(:,4)<10))),' (<10 akt�vnych pr�padov)'])
...
disp(' ')
disp(['Najlep�� fit od ',num2str(fitBeginBest),' dna nakazy v rozmedzi ',num2str(fitTestBegin),'-',num2str(fitTestSpan),' dna az dodnes, kde alpha>0.'])
first=0;
...
serverSave = webwrite(firebase.google.com/APIkey=token,pNames[],pValue[values]);
...
disp(['V�pocet trval ',num2str(round(toc/60*10)/10),' minut'])